    ...
    def _assist(self, recorder, play, deadline):
        ...
        for response in self._assistant.Assist(self._requests(recorder), deadline):
            ...
            if response.speech_results:
                ...
                text = ''.join(r.transcript for r in response.speech_results)
                text.strip()
                print('*****', text)
                if text in ['내 얘기 들어 봐','내 얘기 좀 들어 줘','얘기 들어 봐','얘기 들어 줘','내 얘기 좀 들어 봐']:
                    print('get here!!')
                    return False, True
    ...
    def conversation(self, deadline=DEFAULT_GRPC_DEADLINE):
        ...
        while keep_talking:
            ...
            with Recorder() as recorder, BytesPlayer() as player:
                ...
                try:
                    keep_talking, listen_flag = self._assist(recorder, wrapped_play, deadline)
                finally:
