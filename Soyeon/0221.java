 ...
 * 첫 번째 -> 마지막 칸에 다다를수 있는지 : boolean toEnd(int[]) --> O(N^2) --> O(N) 가능
 * 첫 번째 -> 마지막 칸에 다다르는 최소의 점프 수: int minJump(int[]) --> O(N^4) --> O(N^2) 가능???
 **/
import java.util.*;
public class Main {
	...
	public static boolean toEnd2(int[] list){
		//[2, 0, 1, 1, 2] -> true
		//[1, 0, 2, 1, 3] -> false
		int reachable = 0;
		//시간 복잡도 O(N)
